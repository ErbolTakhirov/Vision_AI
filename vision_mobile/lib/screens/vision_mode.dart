...
class VisionModeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (cameraController != null && cameraController!.value.isInitialized) {
      ...
      // Camera aspect ratio is usually width/height of the sensor (landscape)
      // We need to invert it for portrait calculation or check orientation
      final cameraRatio = cameraController!.value.aspectRatio;
      
      // Calculate scale to cover the screen
      scale = 1 / (cameraRatio * deviceRatio);
      // If scale is still causing boundaries, we might need to flip logic based on orientation
      // A robust generic way for Portrait apps:
    ...
    return Stack(
      fit: StackFit.expand,
      children: [
        // 1. Camera Feed (Fixed with Transform.scale for Cover effect)
        Transform.scale(
          scale: scale,
